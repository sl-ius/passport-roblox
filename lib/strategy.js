...
function Strategy(options, verify) {
  ...
  options.authorizationURL || 'https://authorize.roblox.com';
  options.tokenURL =
  options.tokenURL || 'https://apis.roblox.com/oauth/v1/token';
...
Strategy.prototype.userProfile = function (accessToken, done) {
  ...
  this._oauth2.get(
    ...
    function (err, body, res) {
      if (err) {
        return done(
          new InternalOAuthError('Failed to fetch the profile.', err)
        );
      }
